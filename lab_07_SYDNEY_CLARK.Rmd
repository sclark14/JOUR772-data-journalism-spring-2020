...
```{r}
# Comment- for the buyer_state column, it's only showing PR (Puerto Rico), which is a U.S. territory but not one of the 50 states in the U.S. In the pills table, actual U.S. states appear in the buyer_state column. Additionally in the not in population table, the countyfips column is showing N/A values. That could cause another problem in the future.
```
...
```{r}
# Define API Key
census_api_key("7e928c3d2fbc35e6bb1740378f03cf61d783951f")
...
geo_pills_population_state_2009 %>%
  ...
  geom_sf(lwd = 0) +
  theme_map() +
  labs(fill='Pills per Person (2009)',title="Appalachia had high rates of opioid shipments", subtitle = "In 2009, the amount of opioid shipped per person was the highest in the following states: West Virginia, Kentucky", caption = "Source: ARCOS database") +
...
geo_pills_population_state %>%
  ...
  geom_sf(lwd = 0) +
  theme_map() +
  labs(fill='Pills per Person',title="Starting slow, shipments of pills rapidly increased between 2006 and 2012", subtitle = "Pills per person shipped to each state, 2006 to 2012", caption = "Source: ARCOS database") +
  theme(legend.position="bottom") +
  scale_fill_viridis_c(option = "magma",labels = comma)
...
```{r}
#I want to filter the population table just for 2009. 
...
# I want to filter pills population table to just include Maryland. 
...
# Then, I want to join it to county shifted shapefile
...
# Finally, I want to draw the facet map
