...
# I see a trend: the lower the median household income (value), the greater number of pills per person.
```
...
#I think I notice a trend, a negative relationship happening due to the direction of the line on the scatterplot; median household income vs. pills - the higher the income, the lower the rate per person; most of the dots are showing in the 0 to 100 section on the graph. 
...
#So when you test the relationship, we get this...

cor.test(correlation_pills_income$median_household_income, correlation_pills_income$average_pills_per_person)

#It's giving me negative values, so I think more information is needed to reflect positive values if we want to prove the trend from The Washington Post article. 
```
