...
mingo_working <- mingo %>%
  mutate(state_distribution = case_when(
    reporter_address1 > 71 ~ "in_state_distribution",
    reporter_address1 < 71  ~ "out_of_state_distribution",)
  )
...
mingo %>%
  ...
  summarise(shipments = n(),
            ...
            min_pills_in_shipment = min(dosage_unit),
            max_pills_in_shipment = max(dosage_unit),
            median_pills_in_shipment =max(dosage_unit)
            ) %>%
  arrange(desc(max_pills_in_shipment))
...
# Answer - Actavis Pharma, Inc. MIAMI-LUKEN and STROSNIDER
...
# My suggestion is to create some type of pivot table that includes Cardinal Health (reporter_name) with maybe a $ at the end to signal all contents having the same name, similar to what we did from last week's homework lab. 
